...
```{r data acquisition, echo=FALSE, warning=FALSE}
...
In contrast, performance, job satisfaction and life satisfaction are highley skewed except PERF09 and PERF11. The turnover variables all show little or zero variance.
...
Importantly, some of the turnover variables (TO08, TO09) yield no correlations - this can be explained by their nearly zero variance pattern.


```{r corrplot1, fig.height=6, fig.width=6, cache=FALSE, echo=FALSE}
dataset.items %>% 
...
The performance variables PERF07, PERF08, PERF09 show correlations in increasing order with any Big5 composite score. 
...
Social desirability (sd) correlates strongly negatively with neuroticism, interestingly, and moderately positively with conscientiousness. 
Taken together, social desirability and performance only show a weak positive relationship.
